...
export default function SidebarDashboardPage() {
    const [activeView, setActiveView] = useState<'home' | 'transaction' | 'payout' | 'settings'>('home');
        ...
        <div className="relative w-full min-h-[calc(100vh-80px)] md:h-[calc(100vh-80px)] overflow-x-hidden md:overflow-hidden bg-gradient-to-br from-blue-50 via-white to-blue-100 flex flex-col md:flex-row">
            ...
            <div className="flex-1 p-6 md:p-10 relative z-10 md:h-full overflow-y-auto">
                <AnimatePresence mode="wait">
                    {activeView === 'home' && <HomeView key="home-view" setActiveView={handleNav} />}
                    {activeView === 'transaction' && <TransactionView key="transaction-view" />}
                    {activeView === 'payout' && <PayoutView key="payout-view" />}
                    {activeView === 'settings' && <SettingsView key="settings-view" />}
                </AnimatePresence>
            </div>
        </div>
    );
